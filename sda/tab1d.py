    ...
    if n_clicks==0:
        ...
        try:
            ...
            for key in results.keys():
                ...
                try:
                    if (cubename == 'cube1') or (cubename == 'cube1_v2') or (cubename == 'cube2_v2'):
                        error_band_dens = selected['yval_errdens_interpol']
                        error_band_ext = selected['yval_errext_interpol']

                    if (cubename == 'cube2') or (cubename == 'cube3_v2'):
    ...
    try:
        for i in range(len(sc)):
            ...
            if (cubename == 'cube1_v2') or (cubename == 'cube2_v2') or (cubename == 'cube3_v2'):
                try:                
                    # extract the error on the extinction density; used for the differential extinction plot
                    xval_errdens, ycumul_tmp, yval_errdens = reddening(sc[i], cube=globals.cube_errdens2, axes=globals.axes_errdens2, max_axes=globals.max_axes_errdens2,step_pc=step_pc)
                    # extract the error on the intergrated extinction; used for the cumulative extinction plot
                    xval_errext, ycumul_err, yval_errext = reddening(sc[i], cube=globals.cube_errext2, axes=globals.axes_errext2, max_axes=globals.max_axes_errext2,step_pc=step_pc)
                except:
                    err_message = 'error read error cubes'
            ...
            if cubename == 'cube2':
                ...
                try:
                    ...
                    err_message = "error cube 2 (v1)"


            if cubename == 'cube1_v2':
                cube_id = "explore_cube_density_values_010pc_v2.h5"
                xvalues, yvalues_cumul, yvalues = reddening(sc[i], cube=globals.cube_v2_10, axes=globals.axes_v2_10, max_axes=globals.max_axes_v2_10,step_pc=step_pc)
                ...
                try:
                    ...
                    err_message = 'error cube 1 (v2)'

            if cubename == 'cube2_v2':
                cube_id = "explore_cube_density_values_025pc_v2.h5"
                xvalues, yvalues_cumul, yvalues = reddening(sc[i], cube=globals.cube_v2_25, axes=globals.axes_v2_25, max_axes=globals.max_axes_v2_25,step_pc=step_pc)
                ...
                try:
                    ...
                    err_message = 'error cube 2 (v2)'


            if cubename == 'cube3_v2':
                cube_id = "explore_cube_density_values_050pc_v2.h5"
                try:
                    xvalues, yvalues_cumul, yvalues = reddening(sc[i], cube=globals.cube_v2_50, axes=globals.axes_v2_50, max_axes=globals.max_axes_v2_50,step_pc=step_pc)
                except:
                    err_message = "error cube 3 (v2)"
